...
##########################################PREPARE DATASET FOR PATTERN RECONSTRUCTION#################
{
  ...
  input$Response_factor[is.na(input$Response_factor)] <- 0 #TW!this should probably go higher up before the mutate(Sum_response_factor)
}
...
for (sample_name in unique_sample_names) { #TW! This is a long loop..
  ...
  for (sample_name in unique_sample_names) {
    ...
    sample_df <- sample_df |> 
      mutate(
        ...
        `Normalized Area` = as.numeric(`Normalized Area`),
        Relative_distribution = `Normalized Area` / sum(`Normalized Area`, na.rm = TRUE) #TW!Why is this calculated and then below line put as NA
      )
    ...
    for (i in 1:length(sample_df$Type)) {
      ...
      for (z in 1:length(Combinations[1, ])) { 
        ...
        C_1 <- subset(input, subset = (STD_code == Combinations[1, z] & Type == sample_df$Type[i])) #TW! There is no combo of SCCPs and MCCP code and type
        C_2 <- subset(input, subset = (STD_code == Combinations[2, z] & Type == sample_df$Type[i])) #TW! There is no combo of SCCPs and MCCP code and type
