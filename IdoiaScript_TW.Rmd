...
##########################################PREPARE DATASET FOR PATTERN RECONSTRUCTION#################
{
  ...
  input <- input |> 
    group_by(STD_code, Chain_length) |> 
...
for (sample_name in unique_sample_names) {
  ...
  sample_df <- sample_df |> 
    mutate(
      ...
      `Normalized Area` = as.numeric(`Normalized Area`),
      Relative_distribution = as.numeric(0))
  ...
  for (z in 1:length(Combinations[1,])) {
    C_1 <- subset(input, subset = (STD_code == Combinations[1, z])) #input is the standards
    ...
    for (j in 1:100) {
      ...
      print(paste0(sample_name, Distance))
      # Check for NA values in REF or Combo before comparison
      if (!any(is.na(REF)) && !any(is.na(Combo)) && Distance > (sum(sqrt((REF - Combo)^2)))) { #below script activates when Distance > sumofsqrREF-Combo 
...
THANH TEST SCCP Standards
```{r}

Cal_A <- read_excel("./data/OrbitrapDust.xlsx") |>
  mutate(`Analyte Concentration` = as.numeric(`Analyte Concentration`)) |> 
  mutate(`Normalized Area` = as.numeric(`Normalized Area`)) |> 
  mutate(`Normalized Area` = replace_na(`Normalized Area`, 0)) |> # Replace missing values in the Response_factor column with 0
  mutate(Area = replace_na(Area, 0)) |>
